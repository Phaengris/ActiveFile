...
class ActiveFile::Base
  ...
  def initialize(path)
    ...
    self.class.formats.each do |format_class|
      # TODO: method names conflict?
      # TODO: or do it as an accessor like .formats[:format]?
      # TODO: would `as_<format>` give better readability? `as_password_store`, `as_xml_object` etc?
      memoize define_singleton_method format_class.to_s.demodulize.underscore do
        format_class.new(self)
      end
    end
  end

  # TODO: should we provide a File object for direct manipulations?
